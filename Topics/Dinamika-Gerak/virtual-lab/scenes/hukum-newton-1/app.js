...
function getEffectiveForces() {
  if (scenarioSelect.value === "tariktambang") {
    const numLeft = Math.max(
      0,
      Math.min(5, parseInt(numPeopleLeftInput.value) || 0),
    );
    const numRight = Math.max(
      0,
      Math.min(5, parseInt(numPeopleRightInput.value) || 0),
    );
    return {
      f1: numLeft * FORCE_PER_PERSON,
      dir1: "left",
      f2: numRight * FORCE_PER_PERSON,
      dir2: "right",
    };
  } else {
    ...
    return {
      f1: f1,
      dir1: dir1Select.value,
      f2: f2,
      dir2: dir2Select.value,
    };
  }
}
...
function getCurrentFriction(fApp) {
  ...
  if (Math.abs(box.velocity) < 0.001) {
    if (Math.abs(fApp) <= frictionLimit) {
      ...
      fFrictionActual = -Math.sign(fApp || 1) * frictionLimit;
    }
  } else {
    fFrictionActual = -Math.sign(box.velocity) * frictionLimit;
  }
  return fFrictionActual;
}
...
function updatePhysicsState() {
  ...
  let fApp =
    (forces.dir1 === "right" ? forces.f1 : -forces.f1) +
    (forces.dir2 === "right" ? forces.f2 : -forces.f2);

  let fFrictionActual = getCurrentFriction(fApp);
  ...
  return { forces, fApp, fFrictionActual, netForce, frictionLimit };
}

function updatePhysics(dt) {
  if (!isPlaying) return;

  const { fApp, frictionLimit } = updatePhysicsState();
  ...
  if (Math.abs(box.velocity) < 0.001) {
    ...
    let fFrictionActual = -Math.sign(box.velocity) * frictionLimit;
    let netForceTemp = fApp + fFrictionActual;
    let nextVel = box.velocity + (netForceTemp / box.mass) * dt;

    if (Math.sign(box.velocity) !== Math.sign(nextVel)) {
      if (Math.abs(fApp) <= frictionLimit) {
        box.velocity = 0;
      }
    }
  ...
  if (Math.abs(box.velocity) < 1e-4 && state.netForce === 0) {
    box.velocity = 0;
  }
  ...
  if (Math.abs(box.velocity) < 0.01 && netForce === 0) {
    ...
    if (scenario === "tariktambang") {
      conclusionText.textContent =
        "Tarik tambang seimbang (ΣF = 0). Gaya tarik kiri sama kuat dengan gaya tarik kanan, tambang tetap diam.";
    } else if (scenario === "gesekan") {
      conclusionText.textContent =
        `Gaya dorong (${Math.abs(fApp)} N) dilawan sepenuhnya oleh gaya gesek statis (${Math.abs(fApp)} N) sehingga Resultan Gaya ΣF = 0. Meja tetap diam.`;
    } else if (scenario === "berlawanan") {
      conclusionText.textContent =
        "Dua gaya berlawanan memiliki besar yang sama sehingga saling meniadakan (ΣF = 0). Benda tetap diam.";
    } else {
      conclusionText.textContent =
        "Sesuai Hukum I Newton: Benda yang diam akan tetap diam karena Resultan Gaya ΣF = 0.";
    }
  } else if (Math.abs(box.velocity) >= 0.01 && netForce === 0) {
    statusMessage.textContent =
      "Benda Bergerak Kecepatan Konstan (Resultan Gaya = 0)";
    statusMessage.style.borderColor = "#10b981";

    if (scenario === "konstan") {
      conclusionText.textContent =
        `Gaya dorong mesin (${forces.f1} N) seimbang dengan gaya gesek/hambatan (${frictionLimit} N) sehingga Resultan Gaya ΣF = 0. Mobil bergerak konstan dengan kecepatan ${box.velocity.toFixed(2)} m/s sesuai Hukum I Newton.`;
    } else {
      conclusionText.textContent =
        "Sesuai Hukum I Newton: Benda yang bergerak akan terus bergerak dengan kecepatan konstan karena Resultan Gaya ΣF = 0.";
    }
  } else {
    statusMessage.textContent =
      "Benda Mengalami Percepatan (Resultan Gaya ≠ 0)";
    statusMessage.style.borderColor = "#3b82f6";

    if (scenario === "searah") {
      conclusionText.textContent =
        "Gaya dorong searah menjumlahkan kekuatan (ΣF = F1 + F2). Benda dipercepat maju!";
    } else if (scenario === "berlawanan") {
      conclusionText.textContent =
        "Gaya berlawanan tidak seimbang (ΣF = F1 - F2). Benda dipercepat ke arah gaya yang lebih dominan.";
    } else if (scenario === "gesekan") {
      conclusionText.textContent =
        `Gaya dorong (${Math.abs(fApp)} N) melampaui gaya gesek maksimum (${frictionLimit} N), menghasilkan Resultan Gaya ΣF = ${netForce.toFixed(1)} N. Meja meluncur dipercepat!`;
    } else {
      conclusionText.textContent =
        "Resultan Gaya tidak nol menyebabkan benda mengalami perubahan kecepatan (dipercepat atau diperlambat).";
    }
  }
}
...
function drawScene() {
  ...
  if (isCar || isTug) {
    ...
    ctx.fillRect(0, centerY - 15, canvas.width, 15);
  ...
  if (isCar) {
    ...
    ctx.fillRect(0, centerY, canvas.width, 15);
  } else {
    ctx.beginPath();
  ...
  if (isCar) {
    ...
    scenarioSelect.value === "berlawanan" ||
    scenarioSelect.value === "gesekan"
  ) {
  ...
  if (drawShape === "table") {
    ...
    ctx.beginPath();
    for (
      let tx = boxPixelX - boxPixelW * 3.5;
      tx < boxPixelX + boxPixelW * 3.5;
      tx += 8
    ) {
  ...
  if (scenario === "tariktambang") {
    ...
    scenario !== "konstan" &&
    !(scenario === "custom" && customObject.value === "car")
  ) {
    // Orang Mendorong biasa
    if (forces.f1 > 0) {
      if (forces.dir1 === "right") {
        drawPerson(
          boxPixelX - boxPixelW / 2 - 30,
          personY,
          "right",
          "right",
          "#ef4444",
  ...
  if (forces.f1 > 0) {
    let xStart =
      forces.dir1 === "right"
        ? boxPixelX - boxPixelW / 2
        : boxPixelX + boxPixelW / 2;
    if (scenario === "tariktambang") xStart = boxPixelX;
    
    let label1 = `Aksi 1: ${forces.f1} N`;
    if (scenario === "tariktambang") label1 = `Tarik Kiri: ${forces.f1} N`;
    else if (scenario === "konstan") label1 = `Gaya Mesin: ${forces.f1} N`;

    drawArrow(
      xStart,
      yForceBase,
      forces.f1 * PIXELS_PER_NEWTON,
      forces.dir1,
      "#ef4444",
      label1,
    );
  ...
  if (forces.f2 > 0) {
    ...
    let label2 = `Aksi 2: ${forces.f2} N`;
    if (scenario === "tariktambang") label2 = `Tarik Kanan: ${forces.f2} N`;

    drawArrow(
      xStart,
      yForceBase - 40,
      forces.f2 * PIXELS_PER_NEWTON,
      forces.dir2,
      "#10b981",
      label2,
    );
  ...
  // Gaya Gesek Vector (Di level kontak lantai)
  const fAppCalc =
    (forces.dir1 === "right" ? forces.f1 : -forces.f1) +
    (forces.dir2 === "right" ? forces.f2 : -forces.f2);
  const fFrictionActual = getCurrentFriction(fAppCalc);

  if (Math.abs(fFrictionActual) > 0) {
    const fDir = fFrictionActual > 0 ? "right" : "left";
    const xStart =
      fDir === "right"
        ? boxPixelX - boxPixelW / 2
        : boxPixelX + boxPixelW / 2;
    ...
    drawArrow(
      xStart,
      centerY - 15,
      Math.abs(fFrictionActual) * PIXELS_PER_NEWTON,
      fDir,
      "#8b5cf6",
      `Gaya Gesek: ${Math.abs(fFrictionActual).toFixed(0)} N`,
    );
  }
  ...
  if (Math.abs(netForce) > 0) {
    ...
    drawArrow(
      boxPixelX,
      yForceBase - 80,
      Math.abs(netForce) * PIXELS_PER_NEWTON,
      dir,
      "#f59e0b",
      `ΣF: ${Math.abs(netForce).toFixed(1)} N`,
    );
  }
}
...
scenarioSelect.addEventListener("change", (e) => {
  ...
  switch (e.target.value) {
    case "searah":
      force1Input.value = 100;
      dir1Select.value = "right";
      force2Input.value = 50;
      dir2Select.value = "right";
      frictionForceInput.value = 0;
      break;
    case "berlawanan":
      force1Input.value = 100;
      dir1Select.value = "right";
      force2Input.value = 100;
      dir2Select.value = "left";
      frictionForceInput.value = 0;
      break;
    case "tariktambang":
...
// Clean and validate inputs on input & change
[
  force1Input,
  force2Input,
  frictionForceInput,
...
if (tabP2) {
  tabP2.addEventListener("click", () => {
    alert(
      "Modul Pertemuan 2 (Hukum II & III Newton) sedang dikunci. Silakan selesaikan Pertemuan 1 terlebih dahulu!",
    );
  });
}
